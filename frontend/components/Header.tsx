...
import { PlusIcon, LogoutIcon, MagnifyingGlassIcon, XCircleIcon, HomeIcon, CogIcon, UserGroupIcon, UserCircleIcon, ArrowLeftIcon, SunIcon, MoonIcon, PinIcon, ChevronDownIcon, BillIcon, UserIcon } from './icons';
...
const Header: React.FC<{ appLogic: any }> = ({ appLogic }) => {
    const {
        ...
        searchInputRef, searchContainerRef, settingsMenuRef, roomSelectorRef, roomSelectorInputRef,
        isBrowserSupported,
        handleListen, toggleTheme,
        handleNavigateToUserManagement, handleNavigateToInvoiceManagement, handleNavigateToEditProfile, handleNavigateBackToGrid,
    ...
    return (
        <header className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-lg sticky top-0 z-20 h-16">
            <div className="max-w-7xl mx-auto py-3 px-2 sm:px-6 lg:px-8 flex justify-between items-center gap-2 sm:gap-4 h-full relative">
                {isLoggedIn ? (
                    <>
                        <div className={`flex items-center flex-1 min-w-0 transition-opacity ${isSearchActiveMobile ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                            {isSubPage && canEdit && (
                        ...
                        <div className={`flex flex-shrink-0 justify-end items-center space-x-1 sm:space-x-2 transition-opacity relative z-10 ${isSearchActiveMobile ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                                                        ...
                                                        filteredRoomsForSelector.map((room: any) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex-1 overflow-hidden">
                                                                    ...
                                                                    <p className="text-xs text-slate-500 dark:text-slate-400 group-hover:text-indigo-200 truncate transition-colors duration-150">{room.tenant ? room.tenant.name : 'Phòng trống'}</p>
                                                                </div>
                                                            </div>
                                                        ))
                                                    ) : (
                                            ...
                                            appLogic.searchSuggestions.map((item: any, index: number) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 overflow-hidden">
                                                        ...
                                                        {item.room.tenant && <p className={`text-xs truncate ${highlightedIndex === index ? 'text-indigo-200' : 'text-slate-500 dark:text-slate-400'}`}>{item.room.tenant.name} - {item.room.tenant.phone}</p>}
                                                    </div>
                                                </div>
                                            ))
                                        ) : (
